...
def clean_flux_and_normalize_interactive(
    ...
    cleaned_status = (
        ...
        if (
            ...
            and star.load_property("include_range", epoch_num, band) is not None
        )
        else "X"
    )
    ...
    fig.subplots_adjust(
        left=0.08, right=0.95, top=0.92, bottom=0.25, wspace=0.4, hspace=0.6
    )
    ...
    ax_image.text(
        0.5,
        1.05,
        "Adjust sliders below to select star region",
    ...
    ax_image.text(
        ...
        va="top",
        fontsize=12,
        color="green" if cleaned_status == "V" else "red",
    )
    ...
    if saved_bounds:  # pulled from cache
        ...
        if band == "UVB":
            ...
            centre_idx = len(wavelengths_2D) // 2
            snr_bounds_red = [
                wavelengths_2D[centre_idx - 20],
                wavelengths_2D[centre_idx + 20],
            ]
            snr_bounds_blue = [
                wavelengths_2D[centre_idx - 60],
                wavelengths_2D[centre_idx + 60],
            ]

    # Two red dashed lines  (continuum window)
    snr_red_left = ax_norm.axvline(
        snr_bounds_red[0], color="red", ls="--", lw=1.2, picker=5
    )
    snr_red_right = ax_norm.axvline(
        snr_bounds_red[1], color="red", ls="--", lw=1.2, picker=5
    )

    # Two blue dashed lines (emission‑line window)
    snr_blue_left = ax_norm.axvline(
        snr_bounds_blue[0], color="blue", ls="--", lw=1.2, picker=5
    )
    snr_blue_right = ax_norm.axvline(
        snr_bounds_blue[1], color="blue", ls="--", lw=1.2, picker=5
    )
    ...
    slider_range = RangeSlider(
        ...
        0,
        image_data.shape[0],
        valinit=(default_include_start, default_include_end),
        valstep=1,
    )
    slider_range._handles[0].set_color("red")
    slider_range._handles[1].set_color("red")
    slider_range_bottom = RangeSlider(
        slider_ax_range_bottom,
        "Spacial Range",
        0,
        image_data.shape[0] - 1,
        valinit=(default_bottom_spacial, default_top_spacial),
        valstep=1,
    )
    ...
    finish_button = Button(
        finish_ax, "Finish and Next", color="lightgoldenrodyellow", hovercolor="0.975"
    )
    next_band_button = Button(
        next_band_ax, "Next Band", color="lightblue", hovercolor="0.875"
    )
    prev_band_button = Button(
        prev_band_ax, "Prev Band", color="lightblue", hovercolor="0.875"
    )
    next_epoch_button = Button(
        next_epoch_ax, "Next Epoch", color="lightgreen", hovercolor="0.875"
    )
    prev_epoch_button = Button(
        prev_epoch_ax, "Prev Epoch", color="lightgreen", hovercolor="0.875"
    )
    next_star_button = Button(
        next_star_ax, "Next Star", color="orchid", hovercolor="0.875"
    )
    prev_star_button = Button(
        prev_star_ax, "Prev Star", color="orchid", hovercolor="0.875"
    )
    ...
    (line_summed_vertical,) = ax_summed_vertical.plot(
        [], [], color="blue", label="Summed Flux (vertical)"
    )
    scatter_anchors = ax_summed_vertical.scatter(
        [], [], color="red", label="Anchor Points"
    )
    (line_summed_horizontal,) = ax_summed_horizontal.plot(
        [], [], color="blue", label="Summed Flux (horizontal)"
    )
    (line_norm_cleaned,) = ax_norm.plot(
        [], [], color="blue", label="Cleaned Normalized Summed Flux"
    )
    (line_norm_external,) = ax_norm.plot(
        [], [], color="orange", alpha=0.7, label="Non-Cleaned Normalized Flux"
    )
    ...
    ax_summed_vertical.set_title(
        f"Vertical Summed Flux ({star_name}, {band}, Epoch {epoch_num})", fontsize=10
    )
    ax_summed_horizontal.set_title(
        f"Horizontal Summed Flux ({star_name}, {band}, Epoch {epoch_num})", fontsize=10
    )
    ax_norm.set_title(
        f"Normalized Flux Comparison ({star_name}, {band}, Epoch {epoch_num})",
        fontsize=10,
    )
    ax_diff.set_title(
        f"Flux & Relative Difference ({star_name}, {band}, Epoch {epoch_num})",
        fontsize=10,
    )
    ...
    def onrelease(event):
        ...
        dist = np.hypot(dx, dy)
        movement_threshold = (
            ax_summed_vertical.get_xlim()[1] - ax_summed_vertical.get_xlim()[0]
        ) * 0.01
    ...
    def on_pick(event):
        if event.artist in (snr_red_left, snr_red_right, snr_blue_left, snr_blue_right):
    ...
    def onclick(event_press):
        ...
        if button == 1:
            ...
            if not any(
                [abs(a - anchor_x) < x_threshold for a in selected_anchors_list]
            ):
    ...
    def update_plots(val):
        ...
        if actual_end <= actual_start:
            ...
            anchor_points_in_range = user_anchors[
                (user_anchors >= left_edge) & (user_anchors <= right_edge)
            ]
            tolerance = 15
            has_left_edge_anchor = np.any(
                np.isclose(anchor_points_in_range, left_edge, atol=tolerance)
            )
            has_right_edge_anchor = np.any(
                np.isclose(anchor_points_in_range, right_edge, atol=tolerance)
            )
            ...
            closest_indices = [
                np.abs(wavelengths_2D - ap).argmin() for ap in anchor_points_in_range
            ]
            selected_flux = [
                ut.robust_mean(
                    summed_flux[
                        max(0, idx - mean_flux_half_batch_size) : idx
                        + mean_flux_half_batch_size
                    ],
                    1,
                )
            ...
            if len(all_anchors) > 1:
                continuum_flux_interpolated = np.interp(
                    wavelengths_2D, all_anchors, all_fluxes
                )
            else:
            ...
            if cont_seg.size > 1 and np.all(np.isfinite(cont_seg)):
                ...
                snr_cont = (
                    mu_red / sigma_red
                    if (sigma_red > 0 and np.isfinite(sigma_red))
                    else np.nan
                )
            ...
            if (
                line_seg.size > 1
                and np.all(np.isfinite(line_seg))
                and np.isfinite(sigma_red)
                and (sigma_red > 0)
            ):
        ...
        flux_difference = (
            normalized_summed_flux_resampled - external_normalized_flux_band
        )
    ...
    return (
        final_include_spacial,
        final_bottom,
        final_top,
        navigation,
        results["normalized_summed_flux"],
        results["wavelengths_2D"],
        results["clean_flux"],
        results["interpolated_flux"],
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Interactive flux cleaning and normalization."
    )
    parser.add_argument(
        "--star_names", nargs="+", default=None, help="List of star names to process"
    )
    parser.add_argument(
        "--overwrite_flag",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Flag to create backups before overwriting",
    )
    parser.add_argument(
        "--skip_flag",
        action="store_true",
        default=False,
        help="Flag to skip if results file already exist",
    )
    parser.add_argument(
        "--load_saved_flag",
        action="store_true",
        default=False,
        help="Flag to load saved anchor points and ranges if available",
    )
    ...
    while current_star_idx < len(star_names):
        ...
        epoch_nums = [
            int(re.findall(r"\d+\.\d+|\d+", epoch)[0]) for epoch in epochs_dict.keys()
        ]
        bands = ["UVB", "VIS", "NIR"]
        ...
        while current_epoch_idx < len(epoch_nums):
            ...
            while current_band_idx < len(bands):
                ...
                external_data = star.load_property(
                    "normalized_flux", epoch_num, "COMBINED"
                )
                external_normalized_flux_band = external_data["normalized_flux"]
                external_wavelengths_band = external_data["wavelengths"]

                anchor_points = star.load_property(
                    "norm_anchor_wavelengths", epoch_num, "COMBINED"
                )
                ...
                if (
                    previous_band_height is not None
                    and default_bottom_spacial is not None
                ):
                ...
                (
                    ...
                    continuum_flux_interpolated,
                ) = clean_flux_and_normalize_interactive(
                    image_data,
                    wavelengths_2D,
                    external_normalized_flux_band,
                    external_wavelengths_band,
                ...
                print(
                    f"Selected spacial range (visual): {bottom_spacial} to {top_spacial}"
                )
                ...
                if navigation["finish"]:
                    ...
                    include_range_dict = {
                        "bottom_include": include_spacial[0],
                        "top_include": include_spacial[1],
                    }
                    star.save_property(
                        "include_range",
                        include_range_dict,
                    ...
                    spacial_range_dict = {
                        "bottom_spacial": bottom_spacial,
                        "top_spacial": top_spacial,
                    }
                    star.save_property(
                        "spacial_range",
                        spacial_range_dict,
                    ...
                    star.save_property(
                        "clean_flux",
                        {
                            "clean_flux": clean_flux_before_norm,
                            "wavelengths": returned_wavelengths_2D,
                        },
                        epoch_num,
                        band,
                    ...
                    star.save_property(
                        "interpolated_flux",
                        {"interpolated_flux": continuum_flux_interpolated},
                        epoch_num,
                        band,
                        overwrite=overwrite_flag,
                        backup=backup_flag,
