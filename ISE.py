...
def interactive_normalization(
    ...
    if load_saved:
        try:
            epoch_number = epoch_numbers[
                0
            ]  # Use the first epoch to load the anchor points
            saved_wavelengths = star.load_property(
                "norm_anchor_wavelengths", epoch_number, band
            )
    ...
    def update_plot():
        ...
        if selected_wavelengths_tmp:
            ...
            selected_wavelengths_tmp_index = np.where(
                np.any(
                    np.abs(wavelength[:, None] - selected_wavelengths_tmp) <= tolerance,
                    axis=1,
                )
            )[0]
            selected_fluxes_current_epoch = [
                ut.robust_mean(
                    flux[
                        selected_wave_index
                        - mean_flux_half_batch_size : selected_wave_index
                        + mean_flux_half_batch_size
                    ]
                )
                for selected_wave_index in selected_wavelengths_tmp_index
            ]
            ax1.plot(
                selected_wavelengths_tmp,
                selected_fluxes_current_epoch,
        ...
        ax1.set_ylabel("Flux")
        ax1.set_title(
            f"Interactive Normalization - Star: {star.star_name}, Epoch: {epoch_numbers[current_epoch_idx]}, Band: {band}"
        )
        ax1.legend()
    ...
    def plot_interpolated_flux():
        ...
        if len(selected_wavelengths_tmp) >= 2:
            ...
            selected_wavelengths_sorted = np.array(selected_wavelengths_tmp)[
                sorted_indices
            ]
            selected_fluxes_sorted = np.array(selected_fluxes_current_epoch)[
                sorted_indices
            ]
            interpolated_flux = np.interp(
                wavelength, selected_wavelengths_sorted, selected_fluxes_sorted
            )
            ax_mid.plot(
                wavelength,
                interpolated_flux,
            ...
            ax_mid.set_title(
                f"Interpolation - Epoch: {epoch_numbers[current_epoch_idx]}"
            )
        else:
    ...
    def plot_normalized_flux():
        ...
        if len(selected_wavelengths_tmp) >= 2:
            ...
            ax2.plot(
                wavelength, normalized_flux, "-", color="blue", label="Normalized Flux"
            )
            ax2.axhline(
                y=1, color="red", linestyle="--", label="y=1 (Interpolated Flux)"
            )
            ax2.set_title(
                f"Normalized Flux - Epoch: {epoch_numbers[current_epoch_idx]}"
            )
        else:
    ...
    def onclick(press_event):
        ...
        if press_event["button"] == 1:
            ...
            distances = np.hypot(
                candidate_wavelengths - press_event["x"],
                candidate_fluxes - press_event["y"],
            )
    ...
    def finish(event):
        ...
        for epoch_number in epoch_numbers:
            if len(selected_wavelengths_tmp) < 2:
                print(
                    f"Not enough points selected for interpolation in epoch {epoch_number}. Skipping."
                )
                continue
            fits_file = star.load_observation(epoch_number, band)
            ...
            selected_fluxes_epoch = np.array(
                [
                    ut.robust_mean(
                        flux_epoch[
                            selected_wave_index
            ...
            sorted_indices = np.argsort(selected_wavelengths_tmp)
            selected_wavelengths_epoch = np.array(selected_wavelengths_tmp)[
                sorted_indices
            ]
            selected_fluxes_epoch = selected_fluxes_epoch[sorted_indices]
            interpolated_flux_epoch = np.interp(
                wavelength_epoch, selected_wavelengths_epoch, selected_fluxes_epoch
            )
            normalized_flux = flux_epoch / interpolated_flux_epoch
            star.save_property(
                "norm_anchor_wavelengths",
                selected_wavelengths_epoch,
                epoch_number,
                band,
                overwrite=overwrite,
                backup=backup,
            ...
            star.save_property(
                "normalized_flux",
                {"wavelengths": wavelength_epoch, "normalized_flux": normalized_flux},
                epoch_number,
                band,
                overwrite=overwrite,
                backup=backup,
            ...
            star.save_property(
                "interpolated_flux",
                {
                    "wavelengths": wavelength_epoch,
                    "interpolated_flux": interpolated_flux_epoch,
                },
                epoch_number,
                band,
                overwrite=overwrite,
...
def filter_f(wavelength, flux, batch_size=6, big_batch_size=10):
    ...
    for i in range(num_batches):
        ...
        big_flux_batch_std = np.min(
            [
                ut.robust_std(flux[i : i + batch_size], 2)
                for i in range(
                    int(np.maximum(0, start_idx - big_batch_size * batch_size / 2)),
                    int(end_idx + big_batch_size * batch_size / 2),
                    batch_size,
                )
            ]
        )
    ...
    small_change_indices = np.where(np.abs(flux_diffs) < big_flux_batch_std_list[:-1])[
        0
    ]
    ...
    if len(small_change_indices) >= 22:
        kept_wavelength_means = kept_wavelength_means[small_change_indices]
        kept_flux_means = kept_flux_means[small_change_indices]
        big_flux_batch_std_list = np.array(big_flux_batch_std_list)[
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Interactive normalization of spectra."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Flag to use the filtering function",
    )
    parser.add_argument(
        "--load_saved_flag",
        action="store_true",
        default=False,
        help="Flag to load saved anchor points if available",
    )
    ...
    while current_star_idx < len(star_names):
        ...
        if skip_flag:
            ...
            for epoch_num in epoch_nums:
                try:
                    result = star.load_property(
                        "norm_anchor_wavelengths", epoch_num, band
                    )
                    if result is None:
                        all_exist = False
                        break
            ...
            if all_exist:
                print(
                    f"Skipping star: {star_name} as results already exist for all epochs."
                )
                current_star_idx += 1
                continue
